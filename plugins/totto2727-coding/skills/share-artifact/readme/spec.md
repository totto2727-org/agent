...
2. For an application surface (`cli`, `agent`, or `gui`), inspect the project's actual published artifacts and document every supported acquisition mode that materially helps users:
   - **Run without permanent installation** — use package-runner or language-native one-off acquisition when it is genuinely supported, such as `npx`, `nix run`, `moonx`, or `go run`.
   - **Install persistently** — use the ecosystem's supported persistent path, such as `npm install --global` (`npm i -g`), `nix profile add` (`nix profile install` is an alias), `moon install`, or a language-specific equivalent.
   - **Use from a consumer flake** — show a complete minimal `flake.nix` when the project exposes a usable flake package or app.
...
For an application render, pass all three template inputs: `temporary_setup_options`, `persistent_setup_options`, and `consumer_flake_setup`. Use an empty list for an unsupported command mode and an empty mapping for an unsupported consumer flake. Each command option identifies the supported route in `description`, its fence language in `language`, and the exact acquisition command in `command`; `consumer_flake_setup` supplies `description` and complete `code` when present. These alternatives are parallel choices, not sequential numbered steps.

Pin every remote executable or installed artifact to the strongest immutable identity the ecosystem supports. Use an exact registry version for commands such as `npx`, `npm install --global`, `moonx`, `moon install`, and `go run`; never render `latest` or an unversioned package. Use an immutable revision for `nix run`, `nix profile add`, and flake input URLs, or show the consumer lock operation and require the resulting `flake.lock` to be committed. When an installer resolves and records an immutable revision itself, state that behavior next to the command. Do not weaken pinning merely to make all acquisition modes look alike.

Do not put product output, behavior verification, authentication flows, or repository preparation such as `git clone`, entering a developer shell, dependency synchronization for contributors, code generation, build verification, tests, lint, CI, or publishing commands in `Setup`. A one-off application command may appear in Setup only as a supported acquisition route; put its representative invocation with inputs, output, effects, and authentication in `Usage` or another appropriate end-user section. Move repository build, test, and operation material to `AGENTS.md`. When a library needs no acquisition or installation step, pass an empty `setup_steps` list; when an application needs none, pass empty values for all three application Setup inputs. The template then renders `No setup is required.` instead of inventing a command.
...
Do not create or keep a nested README unless that path has both distinct user-meaningful Usage, whether owned or directly linked, and an owned API worth documenting. Move reference-only detail to a normal documentation guide instead. MoonBit compilation or doctest convenience is never a reason to keep a README; use test-only dependencies and imports for those checks.
