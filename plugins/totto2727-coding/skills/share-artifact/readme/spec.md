...
For an application render, pass all three template inputs: `temporary_setup_options`, `persistent_setup_options`, and `consumer_flake_setup`. Use an empty list for an unsupported command mode and an empty mapping for an unsupported consumer flake; the template omits those routes without explanation. Each command option supplies its fence language in `language` and exact acquisition command in `command`. An optional `description` is allowed only for a non-obvious, actionable prerequisite or constraint; do not narrate what the command plainly does, implementation history, or why an unsupported route is absent. `consumer_flake_setup` supplies complete `code` when present and may supply the same narrowly scoped optional `description`. These alternatives are parallel choices, not sequential numbered steps.
